...
namespace Htonsv {
	namespace Protocol {
		...
		inline Item Methods(unsigned int id) {return find(_method_map, id);}
		inline Item Methods(Method method) {return find(_method_map, static_cast<unsigned int>(method));}
		inline Item Methods(const std::string& name) {return find(_method_map, name);}

		inline Item Codes(unsigned int id) {return find(_code_map, id);}
		inline Item Codes(Code code) {return find(_code_map, static_cast<unsigned int>(code));}
		inline Item Codes(const std::string& name) {return find(_code_map, name);}
	}
}
